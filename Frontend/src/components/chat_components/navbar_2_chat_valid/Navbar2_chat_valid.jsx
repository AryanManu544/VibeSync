...
export default function Navbar2ChatValid() {
  ...
  const leaveOrDelete = useCallback(async (endpoint) => {
    try {
  ...
  const { server_name, categories = [] } = serverDetails;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className={styles.options_comps}><PersonAddIcon fontSize="small" /></div>
        </div>
        {/* Create Category */}
        <div
          className={styles.options}
        ...
        >
          ...
          <div className={styles.options_comps}><CreateNewFolderIcon fontSize="small" /></div>
        </div>
        {/* Leave/Delete */}
        {serverRole === 'author' ? (
          <div
          ...
          >
            ...
            <div className={styles.options_comps}><DeleteForeverIcon fontSize="small" /></div>
          </div>
        ) : (
          <div
            className={styles.options}
          ...
          >
            ...
            <div className={styles.options_comps}><LogoutIcon fontSize="small" /></div>
          </div>
        )}
      </div>
      ...
      <Modal
        ...
        keyboard={false}
        centered
        contentClassName={styles.invite_modal_content}
      >
        <div className={styles.invite_card}>
          {/* Header */}
          <div className={styles.invite_card_header}>
            <Typography className={styles.invite_card_title}>
              Invite Friends to {server_name}
            </Typography>
            <CloseIcon
              className={styles.invite_card_close}
              onClick={() => setShowInviteModal(false)}
            />
          </div>
          {/* Body */}
          <div className={styles.invite_card_body}>
            {inviteLink ? (
              <code className={styles.invite_card_link}>{inviteLink}</code>
            ) : (
              <Skeleton variant="text" width={200} />
            )}
            <button
              className={styles.invite_card_copy}
