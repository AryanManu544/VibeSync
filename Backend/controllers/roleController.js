...
exports.assignRole = async (req, res) => {
  ...
  try {
    ...
    const roleExists = server.roles.some(r => r.id === role_id);
    if (!roleExists) return res.status(404).json({ message: 'Role not found in server' });

    const serverUserIndex = server.users.findIndex(u => u.user_id === user_id);
    if (serverUserIndex === -1) return res.status(404).json({ message: 'User not found in server' });
    ...
    if (userServerIndex === -1) return res.status(404).json({ message: 'Server not found in user.servers' });
    ...
    // Mark subdocuments as modified so Mongoose knows to save them
